...
fun visualComponentSetterNMS(item: Any, sItem: ModernSItem): Any {
    ...
    filtered["visual:material"]?.let {
        val bukkitItem = asBukkitCopy(item)
        bukkitItem.type = XMaterial.valueOf(it.toString()).get() ?: Material.STONE
        filtered.remove("visual:material")
        resultItem = asNMSCopy(bukkitItem)
    }
