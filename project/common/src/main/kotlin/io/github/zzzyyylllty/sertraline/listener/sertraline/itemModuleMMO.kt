...
val mmoFilter = listOf("id", "source", "slot", "allowed", "fix-attack-speed")
        ...
        for (attribute in it) {
            ...
            if (mmoFilter.contains(key)) {
                when (key) {
                    ...
                    "allowed" -> e.itemData["mmo:${key}"] = attribute.value.asListEnhanced() ?: listOf<String>()
                    "fix-attack-speed" -> e.itemData["mmo:${key}"] = attribute.value.toBooleanTolerance()
                }
            } else {
